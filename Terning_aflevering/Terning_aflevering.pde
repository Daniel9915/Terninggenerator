...
void keyPressed() {
  ...
  if ((key=='k' || key == 'K') && counter < 30) {
    counter++;                               //Counter stiger ved at klikke K.
...
int lavTerningeKast(int min, int max){        
  ...
  numbers[counter] = nyKast;                 //Sætter "nyKast" ind i arrayet.
...
void fjernSidsteKast(){
  if(numbers[counter]==1)ones--;                    //Fjerner sidste tal fra 'forskellige tal tælleren'
